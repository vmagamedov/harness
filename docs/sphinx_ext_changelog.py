    ...
    def run(self):
        ...
        for base_version in sorted(by_base_version, reverse=True):
            ...
            if (
                ...
                and CURRENT_VERSION.base_version == str(base_version)
            ):
                title = f'v{base_version} (dev)'
            else:
                title = f'v{base_version}'
            version_section = nodes.section('', nodes.title(title, title),
                                            ids=[ident])
            ...
            for change in by_base_version[base_version]:
                ...
                change_p[0].insert(0, nodes.raw(
                    text='[<span style="color: {};">{}</span>]'.format(
                        _COLORS[change.type],
                        change.type.name.capitalize(),
                    ),
                    format='html',
                ))
...
class Change(SphinxDirective):
    ...
    def run(self):
        self.env.temp_data['x-changes'].append(ChangeEntry(
