...
def _render_wires(proto_file: str, spec: ConfigSpec) -> Tuple[str, str]:
    buf = Buffer()
    ...
    with buf.indent():
        ...
        for wire in spec.wires:
            if wire.value.WhichOneof("type") == "input":
                if wire.optional:
                    type_annotation = f"Optional[{wire.value.input.type}]"
                else:
                    type_annotation = wire.value.input.type
                buf.add(f"{wire.name}: {type_annotation}")
                empty = False
        if empty:
            buf.add("pass")
    buf.add("")
...
def _render_entrypoint(proto_file: str) -> Tuple[str, str]:
    buf = Buffer()
    buf.add("# Generated by the Protocol Buffers compiler. DO NOT EDIT!")
    buf.add(f"# source: {proto_file}")
    buf.add(f"# plugin: {__name__}")
...
def render(proto_file: str, spec: ConfigSpec) -> Generator[Tuple[str, str], None, None]:
